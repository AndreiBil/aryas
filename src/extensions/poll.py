...
async def poll(ctx: commands.Context) -> None:
    ...
    await bot.delete_message(ctx.message)  # Delete the command message to avoid redundancy.
    ...
    msg: discord.Message = await bot.say("", embed=em)

    to_delete = await bot.say("*Setting up poll; please wait...*")
    for emoji in answer_emojis:  # Add possible answer emojis
        await bot.add_reaction(msg, emoji)
    await bot.delete_message(to_delete)

    await sleep(timeout)

    totals: List[int] = [0 for _ in range(len(answer_emojis))]
    # TODO: Check whether the `Message` object auto-updates
    updated_msg = bot.get_message(channel=msg.channel, id=msg.id)
